...
def searchTitle(keyword):
    ...
    for item in bookElements:
        strTitle = item.findtext('publicDataSj')
        if (strTitle.find(keyword) >=0 ):
            print(strTitle)
            
            
def descrip(keyword2):
     #get Book Element
    global strxml
    from xml.etree import ElementTree
    tree=ElementTree.fromstring(strxml)
    bookElements = tree.getiterator('item')  # return list type
    for item in bookElements:
        strTitle = item.findtext('oprtinNm')
        if(strTitle == keyword2 ):
            strTitle2 = item.findtext('svcDc')
            #if (strTitle2.text.find(keyword2) >=0 ):
            print(strTitle2)
